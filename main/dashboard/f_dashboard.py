...
def dashboard_page(product_id=None):
    ...
    # Placeholders for instructions and particles
    ...
    with center:
        ...
        with st.sidebar.expander("Craft Your Investment Strategy", expanded=True):
            ...
            ai_context_input = ai_strategy_context()

        # ... continue with the rest of your dashboard logic, plots, backtests, etc. ...
        ...
        def portfolio_allocation_fragment():
            ...
            with st.expander("Portfolio Allocation Settings", expanded=False):
                ...
                include_single_stock = (
                    st.selectbox("Plot Single Assets?", ["Yes", "No"], 1) == "Yes"
                )
            ...
            with st.expander("Set Benchmark", expanded=False):
                return (
                    st.text_input(
                        ...
                        value="SPY",
                        placeholder="Ex: SPY",
                        help="Benchmark asset symbol (e.g., SPY).",
                    )
                    .strip()
                    .upper()
                )
        ...
        with st.sidebar:
            ...
            # ---------------- Set Your Timeline ----------------
            @st.fragment
            def timeline_fragment(tickers=None, benchmark=None):
                """Render the timeline selector and return the chosen start/end dates."""
                tickers = tickers or []        # avoid NameError on first run
                    ...
                    def generate_months(start_date, end_date):
                        ...
                        while current <= end_date:
                            ...
                            current = (current.replace(year=current.year + 1, month=1)
                                    if current.month == 12 else
                                    current.replace(month=current.month + 1))
                        return months

                    # --- earliest date logic (unchanged) ---
                    earliest_dates = []
                    if tickers:
                        earliest_dates += [d for d in
                                        (get_earliest_date(t) for t in tickers) if d]
                    bd = get_earliest_date(benchmark)
                    if bd: earliest_dates.append(bd)
                    restricted_start_date = max(earliest_dates) if earliest_dates else datetime(2010, 1, 1).date()

                    # --- build month list + defaults (unchanged) ---
                    months = generate_months(restricted_start_date, datetime.today().date())
            ...
            # ───────────────────────── 2.  Call it inside the sidebar ─────────────────────────
            with st.sidebar:
                start_date, end_date = timeline_fragment(tickers, benchmark)
        ...
        @st.fragment
        def custom_asset_weights():
            # 1) Always define these up‐front
            weights = {}
            ...
            if tickers:
                ...
                # 2) Now overwrite it when you actually have values
                ...
                # 3) Even here, total_custom_weight is defined (0.0)
                st.info("No assets selected. Please enter tickers above to assign weights.")

            return weights, total_custom_weight

        with st.sidebar.expander("Enter The Details", expanded=False):
            custom_asset_weights, total_custom_weight = custom_asset_weights()
        ...
        if st.session_state.get("simulation_ran", False):
                ...
                if st.session_state.get("simulation_ran", False):
                    ...
                    if user_question:
                        with st.spinner("Generating insights..."):
                            try:
                                # Call the centralized function from b_nlp_engine.py
                                response = query_gpt(
            ...
            def get_download_link(data, filename, mime, label):
                """Generate a styled HTML download link using a styled <a> tag."""
            ...
            @st.fragment
            def download_section(simulation_data=simulation_data):
                # --- Download Section ---
                st.markdown(
                    '<h2 style="margin-top: 80px;">Portfolio Data Download</h2>',
                    unsafe_allow_html=True
                )

                # Prepare Portfolio Download Data (Excel)
                portfolio_excel = prepare_portfolio_download(
                    ...
                    # New parameters for additional portfolios
                    ...
                    correlation_matrix = simulation_data["returns"][simulation_data["tickers"]].corr()
                                        if simulation_data["tickers"]
                                            ...
                                            and all(s in simulation_data["returns"].columns
                                                    for s in simulation_data["tickers"])
                                        else None
                )

            # Prepare Efficient Frontier Data (CSV)
